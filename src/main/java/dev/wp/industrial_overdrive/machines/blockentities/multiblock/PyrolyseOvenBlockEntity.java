...
public final class PyrolyseOvenBlockEntity extends AbstractElectricMultipliedCraftingMultiblockBlockEntity {
    ...
    public static void initTiers() {
        ...
        HatchFlags hatches = new HatchFlags.Builder().with(HatchType.FLUID_INPUT, HatchType.FLUID_OUTPUT,HatchType.ITEM_INPUT, HatchType.ITEM_OUTPUT, HatchType.ENERGY_INPUT).build();
            ...
            for(int z = 0; z < 4; z++) {
                ...
                for(int x = -1; x <= 1; x++) {
                    for(int y = -1; y <= 1; y++) {
                        ...
                        // If not front or back, use coils, else use bricks for member, hatches go on front and back
                        builder.add(x, y, z, isFront || isBack ? casing : coil, isFront || isBack ? hatches : null);
                    }
                }
            }
